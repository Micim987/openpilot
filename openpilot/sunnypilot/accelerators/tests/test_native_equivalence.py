...
A device with comma's board fitted must behave exactly as it does on NRDR's
...
the footprint: chestnut statements are NRDR's byte for byte, and the module
...
# Preserve NRDR's behavior, including its Honda and model timing changes, rather
# than comparing this port against ZoomPilot's parent branch.
BASELINE = 'origin/nrdr-architecture-development'
...
class UpstreamFootprint(unittest.TestCase):
  """modeld.py stays five hunks wide, and native lines stay NRDR's."""
  ...
  def test_the_fallback_is_the_baseline_behind_one_guard(self):
    ...
    # everything after the guard is NRDR's handler: a small-model fault is
