...
class ModeldV2Seam(unittest.TestCase):
  ...
  def test_nrdr_prepare_only_and_live_delay_paths_survive(self):
    self.assertIn("prepare_only = vipc_dropped_frames > 0", self.src)
    self.assertIn("long_delay = CP.longitudinalActuatorDelay + model.LONG_SMOOTH_SECONDS", self.src)
    self.assertIn("run_mode = (chestnut_state.send if send_chestnut else None) if JETLINK else prepare_only", self.src)
