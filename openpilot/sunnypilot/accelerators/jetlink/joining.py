...
class JoiningModelState:
  ...
  def run(self, bufs, transforms, inputs, after_enqueue=None):
    ...
    try:
      # NRDR's modeld_v2 small model uses this fourth argument as a boolean
      # prepare_only flag, while the JetLink model uses it as a telemetry
      # callback. The callback has nothing to report until the big model is
      # active, so never hand a callable to the small model.
      callback = after_enqueue if active is not self._small else None
      result = active.run(bufs, transforms, inputs, callback)
    except Exception:
