...
class NrdrSelfdrive:
  ...
  def update_personality(selfdrive, CS, button_reserved: bool) -> bool:
    ...
    experimental_mode_switched = selfdrive.experimental_mode_switched
    if distance_released:
      selfdrive.experimental_mode_switched = False

    if not selfdrive.CP.openpilotLongitudinalControl or button_reserved or getattr(selfdrive, "distance_button_consumed", False):
      return False
    ...
    if distance_released and not experimental_mode_switched and (not enabled or consume_button_press(selfdrive.params)):
